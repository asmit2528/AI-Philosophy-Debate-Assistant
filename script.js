// First determine the correct base URL
const API_BASE = window.location.origin.startsWith('http://localhost:5000') 
    ? ''  // If served by Flask, use relative paths
    : 'http://localhost:5000';  // If file://, use absolute
document.getElementById('generateBtn').addEventListener('click', async function() {
    ...
    try {
        ...
        // const response = await fetch(`${API_BASE}/get_arguments`, {
            method: 'POST',
            headers: {
...
// Add this at the beginning of your click event handler, right after the button state change:
document.querySelector('.results').classList.add('visible');

// And modify your displayArguments function to:
function displayArguments(arguments) {
    const output = document.getElementById('argumentsOutput');
    output.innerHTML = '';
